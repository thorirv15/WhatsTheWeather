...
class Forecasts extends StatelessWidget {
  ...
  final List _months = ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'June',
  ...
  Widget build(BuildContext context) {
    ...
    return Expanded(
      child: ListView.builder(
        ...
        DateTime currDay = DateTime.fromMillisecondsSinceEpoch(
            _forecast[index]['dt'] * 1000);
          ...
          return Card(
            ...
            child: Column(
              children: <Widget>[
                ...
                // The details of the forecast are displayed in 
                // the ForecastDetails widget.
                  ...
                  child: Row(
                    children: <Widget>[
                      ...
                      weatherDescriptionContainer(
                          ...
                          _forecast[index]['weather'][0]['main'], 30, 0, 0)
                    ],
                  )
                ),
                // The details of the forecast are displayed in 
  ...
  Widget infoRow(DateTime currDay, List<String> dateStrings, int index) => Row(
    children: <Widget>[
      tempContainer( _forecast[index]['todayForecast']['main']['temp'], 60, 3),
      weatherDescriptionContainer(
          _forecast[index]['todayForecast']['weather'][0]['icon'],
          _forecast[index]['todayForecast']['weather'][0]['main'], 25, 10, 8),
      Column(
        children: <Widget>[
  ...
  Widget dateContainer(DateTime currDay, List<String> dateStrings, 
      double marginL, double marginT) => Container(
    margin: EdgeInsets.fromLTRB(marginL, marginT, 0, 0),
    child: Column(
      children: <Widget>[
        Text(_weekDays[currDay.weekday - 1], style: TextStyle(fontSize: 16.0)),
        Text(dateStrings[2] + ' ' + _months[int.parse(dateStrings[1]) - 1] + ' ' +
              dateStrings[0])
      ]
    )
  );

  Widget tempContainer(var tempature, double marginL, double marginT) => Container(
    margin: EdgeInsets.fromLTRB(marginL, marginT, 0, 0),
    child: Text(kelvinToCelciusStr(tempature), style: TextStyle(fontSize: 20.0))
  );

  Widget weatherDescriptionContainer(String icon, String descr, double marginL, 
      double marginT, double marginB) => Container(
  ...
  Widget highLowTempContainer(int index, var max, var min, 
      double marginL, double marginT) => Container(
    margin: EdgeInsets.fromLTRB(marginL, marginT, 0, 0),
    child: Column(
      children: <Widget>[
        Text('High: ' + kelvinToCelciusStr(max), style: TextStyle(fontSize: 12.0)),
        Text('Low: ' + kelvinToCelciusStr(min), style: TextStyle(fontSize: 12.0))
      ]
    )
  );
}
